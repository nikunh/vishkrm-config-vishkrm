...
# Vishkrm Configuration Utility - Features Module
...
list_installed_features() {
    ...
    echo ""
    if command -v /usr/local/lib/vishkrm-config/modules/feature-update-checker.sh &>/dev/null; then
        /usr/local/lib/vishkrm-config/modules/feature-update-checker.sh list
    else
...
check_feature_updates() {
    ...
    echo ""
    if command -v /usr/local/lib/vishkrm-config/modules/feature-update-checker.sh &>/dev/null; then
        /usr/local/lib/vishkrm-config/modules/feature-update-checker.sh status
    else
        style_error "❌ Feature update checker not available"
    fi
...
force_feature_update_check() {
    ...
    if command -v /usr/local/lib/vishkrm-config/modules/feature-update-checker.sh &>/dev/null; then
        /usr/local/lib/vishkrm-config/modules/feature-update-checker.sh force
        echo ""
        /usr/local/lib/vishkrm-config/modules/feature-update-checker.sh status
    else
        style_error "❌ Feature update checker not available"
...
view_update_status() {
    ...
    if command -v /usr/local/lib/vishkrm-config/modules/feature-update-checker.sh &>/dev/null; then
        local prompt_status=$(/usr/local/lib/vishkrm-config/modules/feature-update-checker.sh prompt 2>/dev/null)
